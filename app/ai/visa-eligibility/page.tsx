...
export default function VisaEligibilityTool() {
  ...
  const calculate = () => {
    ...
    let score = 100;
    const notes: string[] = [];
    if (ageNum >= 3) { score -= 25; notes.push("Some routes cap at 45 (e.g. German skilled worker)."); }
    if (level === "Secondary") { score -= 15; notes.push("A post-secondary degree unlocks more routes."); }
    if (exp === "0–2 years") { score -= 10; notes.push("More experience strengthens skilled work / PR files."); }
    if (lang.startsWith("A1")) { score -= 10; notes.push("A higher language level boosts points (German/English)."); }
    if (country === "Saudi Arabia" && !exp.startsWith("6+")) { score -= 5; }
    const missing = Math.min(score, Math.max(67 - (score / 100) * 67, 0));
    const status = score >= 85 ? "Strong profile" : score >= 65 ? "Good, needs polish" : "Needs a plan";
    setResult({ score, verdict: status, notes: [`Estimated eligibility score: ${score}/100.`, ...notes] });
  };
    ...
    <section className="relative overflow-hidden bg-white py-28 sm:py-36 dark:bg-navy-950">
      ...
      <Container className="relative">
        ...
        <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.1] text-navy-900 text-balance sm:text-5xl dark:text-white">
          Visa <span className="text-gradient-gold">Eligibility</span> Check
        </h1>
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-navy-600 dark:text-navy-100/75">
          Answer 5 quick questions for a free honesty-first estimate of your options. Your counsellor refines the final route.
        </p>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ...
            <label key={field.label} className="block">
              <span className="text-xs font-medium uppercase tracking-[0.12em] text-navy-500 dark:text-navy-200/60">{field.label}</span>
              <select
                value={field.value}
              ...
              >
                {field.options.map((o) => (
                  <option key={o} className="bg-white text-navy-900 dark:bg-navy-950 dark:text-white" value={o}>{o}</option>
                ))}
              </select>
            </label>
          ))}
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <p className="text-xs text-navy-500 dark:text-navy-200/50">Estimates only — always confirm with a certified counsellor.</p>
        </div>
        ...
        {result && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-10 rounded-3xl border border-gold-500/30 bg-gold-50/30 p-8 backdrop-blur-md dark:border-gold-400/30 dark:bg-white/5">
            <p className="flex items-center gap-2 font-display text-2xl font-bold text-navy-900 dark:text-white">
              <CheckCircle2 className="size-6 text-gold-500 dark:text-gold-400" />
              {result.verdict} — {result.score}%
            </p>
            <ul className="mt-4 space-y-2">
              {result.notes.map((note) => (
                <li key={note} className="text-sm leading-relaxed text-navy-600 dark:text-navy-100/75">{note}</li>
              ))}
            </ul>
            <p className="mt-4 text-sm text-navy-500 dark:text-navy-200/60">
              This is a directional score. Book a free consultation and we will map the precise route, points and documents.
            </p>
            <div className="mt-6">
              <Button variant="glass" href="/apply" withArrow>Book Free Consultation</Button>
            </div>
          </motion.div>
        )}
      </Container>
