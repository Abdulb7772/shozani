    ...
    <section className="relative overflow-hidden bg-white py-28 sm:py-36 dark:bg-navy-950">
      ...
      <Container className="relative">
        ...
        <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.1] text-navy-900 text-balance sm:text-5xl dark:text-white">
          IELTS <span className="text-gradient-gold">{t("Band Predictor")}</span>
        </h1>
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-navy-600 dark:text-navy-100/75">
          {t("Pick the sentence closest to you in each skill for a quick band direction. See exactly where you are and what to fix.")}
        </p>

        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {Object.entries(sectionSignals).map(([section, options]) => (
            <div key={section} className="rounded-3xl border border-navy-900/10 bg-white p-6 dark:border-white/10 dark:bg-white/5">
              <h2 className="font-display text-lg font-semibold text-navy-900 dark:text-white">{t(section)}</h2>
        ...
        {result !== null && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-10 rounded-3xl border border-gold-700 bg-gold-600 p-8 dark:border-gold-400/30 dark:bg-white/5">
            <p className="flex items-center gap-2 font-display text-2xl font-bold text-white">
              <BarChart3 className="size-6 text-white dark:text-gold-400" />
              {t("Estimated overall band")}: {result.toFixed(1)}
            </p>
            <p className="mt-3 text-sm leading-relaxed text-white/90 dark:text-navy-100/75">
              {result >= 7 ? t("Strong — aim for your target with light polish.") : result >= 6 ? t("Solid — targeted mock practice will lift 0.5–1.0 band.") : t("A structured course with mocks is the fastest fix.")}
            </p>
            <div className="mt-6">
              <Button variant="glass" href="/services/language/ielts" withArrow>{t("Join IELTS Coaching")}</Button>
            </div>
          </motion.div>
        )}
      </Container>
