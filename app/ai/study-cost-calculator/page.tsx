    ...
    <section className="relative overflow-hidden bg-white py-28 sm:py-36 dark:bg-navy-950">
      ...
      <Container className="relative">
        ...
        <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.1] text-navy-900 text-balance sm:text-5xl dark:text-white">
          {t("Study")} <span className="text-gradient-gold">{t("Cost Calculator")}</span>
        </h1>
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-navy-600 dark:text-navy-100/75">
          {t("Tuition, living costs and the blocked account requirement — a realistic yearly and total budget for your destination.")}
        </p>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-3">
          <label className="block">
            ...
            >
              {Object.keys(costData).map((c) => <option key={c} className="bg-white text-navy-900 dark:bg-navy-950 dark:text-white" value={c}>{t(c)}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="text-xs font-medium uppercase tracking-[0.12em] text-navy-500 dark:text-navy-200/60">{t("Duration (years)")}</span>
            <select
              value={duration}
              onChange={(e) => { setDuration(e.target.value); setResult(null); }}
              className="mt-2 w-full rounded-xl border border-navy-900/10 bg-white px-4 py-3 text-sm text-navy-900 outline-none transition-colors focus:border-gold-400/60 dark:border-white/10 dark:bg-white/5 dark:text-white"
            >
              {["2", "3", "4", "5"].map((y) => <option key={y} className="bg-white text-navy-900 dark:bg-navy-950 dark:text-white" value={y}>{y} {t("years")}</option>)}
            </select>
          </label>
          <div className="mt-8 sm:mt-6">
            <Button variant="gold" onClick={calculate} className="w-full">{t("Calculate Budget")}</Button>
          </div>
        </div>
        ...
        {result && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-10 rounded-3xl border border-gold-700 bg-gold-600 p-8 dark:border-gold-400/30 dark:bg-white/5">
            <p className="flex items-center gap-2 font-display text-2xl font-bold text-white">
              <FileText className="size-6 text-white dark:text-gold-400" />
              €{result.perYear.toLocaleString("de-DE", { maximumFractionDigits: 0 })} / {t("year")} — €{result.total.toLocaleString("de-DE", { maximumFractionDigits: 0 })} {t("total")}
            </p>
            <p className="mt-3 text-sm leading-relaxed text-white/90 dark:text-navy-100/75">
              {t("As a rough guide from typical figures.")} {result.blocked > 0 ? `${t("Your visa will need a blocked account of roughly")} €${result.blocked.toLocaleString("de-DE")}.` : t("No blocked account is required for this route, but funds must still be shown.")}
            </p>
            <div className="mt-6">
              <Button variant="glass" href="/services/study-abroad" withArrow>{t("Explore Study Options")}</Button>
            </div>
          </motion.div>
        )}
      </Container>
