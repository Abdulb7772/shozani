...
import { CheckCircle2, BarChart3, Calculator, FileText } from "lucide-react";
      ...
      <section className="relative overflow-hidden bg-white pt-36 pb-20 sm:pt-44 sm:pb-28 dark:bg-navy-950">
        ...
        <Container className="relative">
          ...
          <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.1] text-navy-900 text-balance sm:text-5xl lg:text-6xl dark:text-white">
            {t("Smart tools that")} <span className="text-gradient-gold">{t("de-risk your move")}</span>
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-relaxed text-navy-600 sm:text-lg dark:text-navy-100/75">
            {t("Free, fast and honest estimates — before you spend a single rupee. Every tool is built on the same data our counsellors use.")}
          </p>
        </Container>
      </section>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {tools.map((tool) => (
              ...
              >
                ...
                <h2 className="mt-6 font-display text-xl font-semibold text-navy-900 dark:text-white">{t(tool.title)}</h2>
                <p className="mt-3 text-sm leading-relaxed text-navy-600 dark:text-navy-200/70">{t(tool.description)}</p>
